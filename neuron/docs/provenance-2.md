...
## 7. The first long run

120,000 sentences in one continuous session, 3 hours 23 minutes, 14,843
weight updates.

**Health drifted +0.0723 against a 0.40 tolerance, with 60 guard checks and
zero rollbacks.** Continual learning at that rate does not degrade the
model, and before this it was an assumption rather than a measurement.

Two other things the run showed.

**The gate tightens as the model improves.** Early on roughly one in five
weight-routed items became an update; by the end it was 14,843 from 37,417
routed, with a much flatter recent slope. Less of Wikipedia was surprising
once the model had read a lot of it. Correct behaviour over a timescale
nobody had watched.

**Eviction is still untested.** The store reached 89,456 against a cap of
100,000 and stopped, growing linearly the whole way rather than saturating.
Nothing was evicted.

**And a real bug: 75,000 sentences were stored but not indexed.** `indexed`
read 13,916 against 89,456 stored, because indexing happens lazily on a
search and nothing searched during the run. The first query afterwards has
to encode 75,000 sentences at once, which is the same cost problem that was
fixed for re-encoding and never fixed for deferred indexing. A long-running
system should index as it goes.
...
`long-horizon.md` covers what happened when this runtime's guard was pointed
at a task whose material its canary did not cover: it rolled back the task
itself. Relevant here because the guard is part of the same governance story
as provenance and deletion.
