"""Leave-one-out ablation. neuron.py unchanged; this just varies config.

Four mechanisms went in together. The loss floor fired 3-5 times all run,
so it probably is not the one doing the work. This finds out.

The canary guard is excluded because it never fired (0 rollbacks), so it
has nothing to ablate. It needs its own test.
"""
...
def build_stream(seed):
    ...
    for i in range(STREAM_LEN):
        if i in planted:
            stream.append(planted[i])
        elif rng.random() < 0.12:
            stream.append(make_junk(rng))
        else:
            stream.append(rng.choice(FILLER_POOL))
    return stream
...
# Leave-one-out. Each arm removes exactly one mechanism from the full set.
CONFIGS = {
    "all":       dict(),
    "no_floor":  dict(loss_floor=0.0),
    "no_lrdec":  dict(lr_decay=1.0),
    "no_wdecay": dict(weight_decay=0.0),
    "none":      dict(loss_floor=0.0, lr_decay=1.0, weight_decay=0.0, guard=False),
}

COLORS = {
    "all": "#4ade80", "no_floor": "#fbbf24", "no_lrdec": "#60a5fa",
    "no_wdecay": "#c084fc", "none": "#f87171",
}
...
def run(seed, mode):
    ...
    rows = []
    for i, text in enumerate(build_stream(seed), start=1):
        ai.observe(text)
        if i % CHECK_EVERY == 0:
            row = dict(
                at=i,
            ...
            rows.append(row)
            print(f"    {i:>3}: upd {row['updates']:>3} flr {row['floored']:>3} "
                  f"rb {row['rollbacks']:>2} lr {row['lr']:.2e}  "
...
HTML = """<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>NEURON ablation</title>
<style>
...
<h1>NEURON ablation</h1>
<div class="sub">Each arm removes exactly one stability mechanism. The arm
that degrades most is the one carrying the weight. x axis = items seen.</div>
<div id="charts"></div>
<script>
const DATA = __DATA__;
...
function series(mode, key){
  const runs = DATA[mode], n = runs[0].length, out = [];
  for(let i=0;i<n;i++){
...
function chart(title, desc, key){
  ...
  const W=1000,H=230,ML=58,MR=14,MT=12,MB=26;
  ...
  svg+="</svg>";
  const leg=modes.map(m=>`<span><span class="dot"
    style="background:${COLORS[m]}"></span>${m}</span>`).join("");
...
document.getElementById("charts").innerHTML =
  chart("PROBE — do early facts survive?",
        "Higher is better. Crossing zero means the facts are gone.","probe")
+ chart("DRIFT — is unrelated knowledge safe?",
        "Flat is healthy. Climbing means rot.","drift")
+ chart("FLUENCY — is it still a language model?",
        "Climbing is the fatal case.","fluency")
+ chart("UPDATES","Cumulative weight updates.","updates")
+ chart("FLOORED","Updates skipped because the material was already learned.","floored")
+ chart("DISTANCE","How far the adapters moved from start.","distance");
</script></body></html>"""


results = {m: [] for m in CONFIGS}
print(f"Ablation: {len(CONFIGS)} arms x {len(SEEDS)} seeds x {STREAM_LEN} items\n")
...
with open("ablation.json", "w") as f:
    json.dump(results, f)
with open("ablation.html", "w") as f:
    f.write(HTML.replace("__DATA__", json.dumps(results))
                .replace("__COLORS__", json.dumps(COLORS)))

print("=" * 78)
print(f"{'arm':>10} {'upd':>6} {'flr':>5} {'rb':>4} {'probe':>9} "
      f"{'drift':>9} {'fluency':>9}")
print("-" * 78)
for m in CONFIGS:
    last = [r[-1] for r in results[m]]
    f_ = lambda k: sum(r[k] for r in last) / len(last)
    print(f"{m:>10} {f_('updates'):>6.1f} {f_('floored'):>5.1f} "
          f"{f_('rollbacks'):>4.1f} {f_('probe'):>+9.4f} "
          f"{f_('drift'):>+9.4f} {f_('fluency'):>+9.4f}")
print("=" * 78)
print("""
The arm closest to 'none' is the mechanism that was carrying the load.
The arm closest to 'all' is a mechanism you can delete.

Run:  open ablation.html
""")